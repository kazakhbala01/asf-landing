      ...
      <div className="mx-auto max-w-[85rem] px-4 py-14 sm:px-6 lg:py-18">
        <div className="grid grid-cols-1 items-start gap-10 lg:grid-cols-[1fr_2fr]">
          ...
          <ul className="grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-3">
            {points.map((p) => (
              <li key={p.title} className="reveal flex flex-col items-center gap-4 text-center">
                <span className="flex h-32 w-32 items-center justify-center rounded-full bg-amber/10 text-amber-dark ring-1 ring-amber/25 transition-colors hover:bg-amber/20 sm:h-36 sm:w-36">
                  {p.icon}
                </span>
                <span className="max-w-[180px] text-[15px] font-bold leading-snug">
                  {p.title}
                </span>
              </li>
            ))}
